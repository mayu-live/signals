    ...
    module Core
      class Root
        ...
        def self.create(&block) =
          Utils.with_fiber_local(CURRENT_KEY, new, &block)
        ...
        def end_batch
          ...
          has_error = T.let(false, T::Boolean)
      ...
      class Computed < Signal
        ...
        def _unsubscribe(node)
          ...
          if @targets
            ...
            # Following line uses so sorbet doesn't say: "This code is unreachable"
            unless T.unsafe(@targets)
      ...
      def self.with_eval_context(eval_context, &block) =
        Utils.with_fiber_local(:eval_context, eval_context, &block)
      ...
      def self.cleanup_sources(target)
        node = T.let(target.sources, T.nilable(Node))
